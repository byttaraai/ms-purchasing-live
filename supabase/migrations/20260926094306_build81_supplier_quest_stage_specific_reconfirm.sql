create or replace function purchasing_private.supplier_quest_stage_stale_v81(
  p_stage_data jsonb,
  p_stage text,
  p_dashboard jsonb,
  p_supplier text
...
declare
  saved jsonb;
  saved_norm jsonb := '[]'::jsonb;
  current_norm jsonb := '[]'::jsonb;
  initial_codes jsonb := '[]'::jsonb;
begin
  saved := p_stage_data->p_stage->'snapshot';
  if jsonb_typeof(saved) <> 'array' then
    return true;
  end if;

  if p_stage='data' then
    initial_codes :=
      coalesce(p_stage_data->'_initial_blocking_codes','[]'::jsonb)
      || coalesce(p_stage_data->'_initial_attention_codes','[]'::jsonb);

    select coalesce(jsonb_agg(jsonb_build_object(
      'product_code',x->>'product_code',
      'unit',coalesce(x->>'unit',''),
      'blocking_review',coalesce((x->>'blocking_review')::boolean,false),
      'needs_review',coalesce((x->>'needs_review')::boolean,false),
      'review_reason',coalesce(x->>'review_reason','')
    ) order by x->>'product_code'),'[]'::jsonb)
    into saved_norm
    from jsonb_array_elements(saved) x;

    select coalesce(jsonb_agg(jsonb_build_object(
      'product_code',x->>'product_code',
      'unit',coalesce(x->>'purchase_unit',''),
      'blocking_review',coalesce((x->>'blocking_review')::boolean,false),
      'needs_review',coalesce((x->>'needs_review')::boolean,false),
      'review_reason',coalesce(x->>'review_reason','')
    ) order by x->>'product_code'),'[]'::jsonb)
    into current_norm
    from jsonb_array_elements(coalesce(p_dashboard->'rows','[]'::jsonb)) x
    where coalesce(x->>'supplier','')=coalesce(p_supplier,'')
      and (
        coalesce((x->>'needs_review')::boolean,false)
        or initial_codes ? (x->>'product_code')
      );

  elsif p_stage in ('lt10','10_50','50_100','100_150','150_200') then
    select coalesce(jsonb_agg(jsonb_build_object(
      'product_code',x->>'product_code',
      'unit',coalesce(x->>'unit',''),
      'stock_pct',x->'stock_pct',
      'rating',coalesce(x->>'rating',''),
      'price',x->'price',
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'rating',coalesce(x->>'profitability_class',''),
    ...
    where coalesce(x->>'supplier','')=coalesce(p_supplier,'')
      ...
      and (
        (p_stage='lt10' and (x->>'stock_ratio')::numeric<10)
        or (p_stage='10_50' and (x->>'stock_ratio')::numeric>=10 and (x->>'stock_ratio')::numeric<50)
        or (p_stage='50_100' and (x->>'stock_ratio')::numeric>=50 and (x->>'stock_ratio')::numeric<100)
        or (p_stage='100_150' and (x->>'stock_ratio')::numeric>=100 and (x->>'stock_ratio')::numeric<150)
        or (p_stage='150_200' and (x->>'stock_ratio')::numeric>=150 and (x->>'stock_ratio')::numeric<=200)
      );

  elsif p_stage='200_300' then
    select coalesce(jsonb_agg(jsonb_build_object(
      'product_code',x->>'product_code',
      'unit',coalesce(x->>'unit',''),
      'stock_pct',x->'stock_pct',
    ...
    where coalesce(x->>'supplier','')=coalesce(p_supplier,'')
      and jsonb_typeof(x->'stock_ratio')='number'
      and (x->>'stock_ratio')::numeric>200
      and (x->>'stock_ratio')::numeric<=300;

  elsif p_stage='gt300' then
    select coalesce(jsonb_agg(jsonb_build_object(
      'product_code',x->>'product_code',
      'unit',coalesce(x->>'unit',''),
      'stock_pct',x->'stock_pct',
  ...
  return saved_norm is distinct from current_norm;
end;
$function$;

revoke all on function purchasing_private.supplier_quest_stage_stale_v81(jsonb,text,jsonb,text) from public, anon, authenticated;

create or replace function public.purchasing_supplier_quest_v81(payload jsonb)
returns jsonb
language plpgsql
security definer
set search_path to ''
...
begin
  ...
  q.stage_data := jsonb_set(q.stage_data,'{_quest_version}','81'::jsonb,true);
  ...
  foreach k in array stage_keys loop
    if q.stage_data ? k
       and purchasing_private.supplier_quest_stage_stale_v81(q.stage_data,k,d,q.supplier) then
  ...
  if act='state' then
    ...
    foreach k in array stage_keys loop
      ...
      if purchasing_private.supplier_quest_stage_stale_v81(q.stage_data,k,d,q.supplier) then
        raise exception 'Supplier data changed in stage %. Reconfirm that stage first',k;
      end if;
    end loop;
...
revoke execute on function public.purchasing_supplier_quest_v81(jsonb) from public, anon;
grant execute on function public.purchasing_supplier_quest_v81(jsonb) to authenticated;

comment on function public.purchasing_supplier_quest_v81(jsonb) is
'Build 81 Supplier Quest: stage-specific source-change reconfirmation from saved snapshots. Unrelated master revisions do not invalidate completed stages. Does not complete purchasing_tasks_v5 or award badges.';
