...
begin
  ...
  foreach k in array stage_keys loop
    if q.stage_data ? k then
      ...
      if saved_rev is not null and saved_rev<>current_revision then
...
create or replace function public.purchasing_supplier_quest_outputs_v80()
returns jsonb
language plpgsql
security definer
set search_path to ''
...
begin
  ...
  ) order by z.updated_at desc,z.id desc),'[]'::jsonb)
  into out_json
  from (
    select o.*
    from purchasing_private.supplier_quest_outputs_v79 o
    join purchasing_private.supplier_quests_v79 q on q.id=o.quest_id
    where o.user_id=uid and q.user_id=uid and q.status='submitted'
    order by o.updated_at desc,o.id desc
    limit 100
  ) z;

  return jsonb_build_object('outputs',out_json);
end;
$function$;

revoke execute on function public.purchasing_supplier_quest_outputs_v80() from public, anon;
grant execute on function public.purchasing_supplier_quest_outputs_v80() to authenticated;

comment on function public.purchasing_supplier_quest_outputs_v80() is
'Build 80 read-only authenticated worklist feed for Supplier Quest outputs owned by the current purchasing user.';

create index if not exists idx_supplier_quest_outputs_v79_task
  on purchasing_private.supplier_quest_outputs_v79(task_id);

create index if not exists idx_supplier_quest_outputs_v79_upload
  on purchasing_private.supplier_quest_outputs_v79(upload_id);

create index if not exists idx_supplier_quests_v79_upload
  on purchasing_private.supplier_quests_v79(upload_id);
