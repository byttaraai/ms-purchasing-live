...
checks=r"""
  ...
  const riskShareFilter=document.getElementById('overstockRiskShareFilter');
  check([...riskShareFilter.options].map(o=>o.value).join(',')==='all,stable,risky_high,risky_low','Risk Share filter options missing');
  const riskBase=overstockRiskSourceRows(),riskAvg=overstockRiskShareAverage(riskBase);
  check(Number.isFinite(riskAvg)&&riskAvg>0,'Risk Share average missing');
  riskShareFilter.value='risky_high';riskShareFilter.dispatchEvent(new Event('change',{bubbles:true}));
  let riskFiltered=overstockRiskVisibleRows(overstockRiskSourceRows());
  check(riskFiltered.length>0&&riskFiltered.every(r=>r.risk_status==='positive'&&r.risk_share_pct>riskAvg),'Risky High filter failed');
  document.getElementById('overstockRiskShareFilter').value='risky_low';document.getElementById('overstockRiskShareFilter').dispatchEvent(new Event('change',{bubbles:true}));
  ...
  overstockRiskView.supplier='';overstockRiskView.sort='risk_desc';overstockRiskView.rpFilter='all';openOverstockRiskPopup();
  check(!document.getElementById('overstockRpFilter'),'Reorder Point dropdown should be removed');
  check(!document.getElementById('overstockSortSelect'),'Sort By dropdown should be removed');
  ...
  check(overstockRiskView.sort==='risk_desc','Default Overstock sort must be Risk Share');
  const riskSorted=overstockRiskVisibleRows(overstockRiskSourceRows()).filter(r=>r.risk_status==='positive');
  check(riskSorted.every((r,i,a)=>i===0||a[i-1].risk_impact>=r.risk_impact),'Default Risk Share sort failed');
  document.getElementById('overstockQuickRpUnder10').click();
  check(overstockRiskView.rpFilter==='under10','Reorder < 10 quick tag did not activate');
  check(overstockRiskVisibleRows(overstockRiskSourceRows()).every(r=>Number(r.reorder_point)>0&&Number(r.reorder_point)<10),'RP under-10 quick tag failed');
  document.getElementById('overstockQuickRpUnder10').click();
  check(overstockRiskView.rpFilter==='all','Reorder < 10 quick tag did not reset');
  document.getElementById('overstockQuickExcessSort').click();
  check(overstockRiskView.sort==='excess_value_desc','Excess Value quick sort did not activate');
  const excessSorted=overstockRiskVisibleRows(overstockRiskSourceRows());
  check(excessSorted.every((r,i,a)=>i===0||(a[i-1].excess_value||0)>=(r.excess_value||0)),'Excess Value quick sort failed');
  document.getElementById('overstockQuickExcessSort').click();
  check(overstockRiskView.sort==='risk_desc','Excess Value quick sort did not return to Risk Share default');
  overstockRiskView.supplier='';openOverstockRiskPopup();
