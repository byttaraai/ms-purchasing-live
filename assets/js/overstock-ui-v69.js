/* Build 69: compact overstock UI; all financial risk comes from OverstockRisk. */
'use strict';
...
function overstockRiskVisibleRows(baseRows){
 ...
 const filtered=baseRows.filter(r=>{
  ...
  if(Number.isFinite(minValue)&&(!C.finite(r.excess_value)||Number(r.excess_value)<=minValue))return false;
  return true;
 });
...
function overstockRiskCell(r){
 ...
 const tone=review?'review':stable?'stable':'positive';
 const exact=Number.isFinite(r.risk_impact)?(r.risk_impact>0?'+':r.risk_impact<0?'\u2212':'')+'SAR '+fmt(Math.abs(r.risk_impact),2):'Not valued';
 const explanation=review?r.risk_reason:stable?'Below the model baseline; still overstock, not a guarantee of safety.':'Share of all positive risk impact in the current BO scope (before popup filters).';
 return`<span class="overstock-risk-cell ${tone}" title="${esc(explanation+' '+exact)}"><strong>${label}</strong><small>${esc(overstockAmount(r.risk_impact))}</small></span>`;
}
...
function openOverstockRiskPopup(options={}){
 ...
 valueInput.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();applyValueFilter();}};
