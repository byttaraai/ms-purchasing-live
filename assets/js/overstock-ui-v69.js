...
'use strict';
const overstockRiskView={supplier:'',riskShareFilter:'all',sort:'risk_desc',rpFilter:'all',minExcessValue:'',maxExcessValue:'',selected:new Set()};
...
function overstockRiskShareAverage(baseRows){
 const risky=baseRows.filter(r=>r.risk_status==='positive'&&C.finite(r.risk_share_pct));
 return risky.length?risky.reduce((n,r)=>n+Number(r.risk_share_pct),0)/risky.length:null;
}
function overstockRiskVisibleRows(baseRows){
 const supplier=overstockRiskView.supplier;
 const riskShareFilter=overstockRiskView.riskShareFilter||'all';
 const riskShareAverage=overstockRiskShareAverage(baseRows);
 ...
 const filtered=baseRows.filter(r=>{
  ...
  if(riskShareFilter==='stable'&&r.risk_status!=='stable')return false;
  if(riskShareFilter==='risky_high'&&(r.risk_status!=='positive'||!C.finite(r.risk_share_pct)||!Number.isFinite(riskShareAverage)||Number(r.risk_share_pct)<=riskShareAverage))return false;
  if(riskShareFilter==='risky_low'&&(r.risk_status!=='positive'||!C.finite(r.risk_share_pct)||!Number.isFinite(riskShareAverage)||Number(r.risk_share_pct)>riskShareAverage))return false;
...
function openOverstockRiskPopup(options={}){
 ...
 $('overstockRiskShareFilter').onchange=e=>{overstockRiskView.riskShareFilter=e.target.value;overstockRiskView.selected.clear();d.dataset.returnScroll='0';openOverstockRiskPopup({resetScroll:true});};
 $('overstockQuickRpUnder10').onclick=()=>{overstockRiskView.rpFilter=overstockRiskView.rpFilter==='under10'?'all':'under10';overstockRiskView.selected.clear();d.dataset.returnScroll='0';openOverstockRiskPopup({resetScroll:true});};
 $('overstockQuickExcessSort').onclick=()=>{overstockRiskView.sort=overstockRiskView.sort==='excess_value_desc'?'risk_desc':'excess_value_desc';d.dataset.returnScroll='0';openOverstockRiskPopup({resetScroll:true});};
