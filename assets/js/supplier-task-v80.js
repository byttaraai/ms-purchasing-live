/* Build 80: resilient Supplier Quest workspace. */
'use strict';
...
async function supplierQuestRpc(task,payload){
  ...
  return rpc('purchasing_supplier_quest_v80',{payload:{task_id:task.id,...payload}});
}
function supplierQuestApplyServer(task,response){
  ...
  const localStale=SupplierQuestModel.staleStages(SupplierQuestUI.quest?.stage_data||{},SupplierQuestUI.masterRevision);
  SupplierQuestUI.staleStages=new Set([...serverStale,...localStale]);
